...
var PriceQueue = {
  ...
  GenPriceDescription: function (rgItems) {
    ...
    if (!(rgItems.lowestPrice || rgItems.nofeePrice)) {
      if (rgItems.marketable && selectmode) {
  ...
  GetPrices: async (
    ...
    try {
      ...
      Promise.all(promises).then(() => {
        const start_index = loadingTotalSteamPrice.findIndex((data) => data.market === market && data.appid === appid);
        if (start_index !== -1) {
...
const loadPriceOverview = (name) => {
  loadPriceSteamInventory(name).then((marketPrice) => {
    g_ActiveInventory.m_rgItemElements.forEach((item) => {
      const des = item[0].rgItem.description;

      if (des.market_hash_name === name) {
        des.lowestPrice = marketPrice.lowest_price;
        des.nofeePrice = marketPrice.median_price;
        des.volume = marketPrice.volume;
      }
    });
