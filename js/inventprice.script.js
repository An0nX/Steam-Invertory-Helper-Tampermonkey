...
CInventory.prototype.AddInventoryData = function (data) {
  ...
  if (data.descriptions) {
    for (var i = 0; i < data.descriptions.length; i++) {
      ...
      if (description.instanceid && description.instanceid != '0') key += '_' + description.instanceid;

      if (!this.m_rgDescriptions[key]) {
        if (!description.tags) description.tags = [];

        if (!g_bIsTrading && !g_bShowTradableItemsOnly) {
          if (description.tradable) description.tags.push(kStandardTag_Tradable);
          else description.tags.push(kStandardTag_Untradable);
        }

        if (g_bUseMarketLinks) {
          if (description.marketable) description.tags.push(kStandardTag_Marketable);
          else description.tags.push(kStandardTag_Unmarketable);
        }
  ...
  if (data.assets) {
    for (var i = 0; i < data.assets.length; i++) {
      ...
      if (asset.instanceid && asset.instanceid != '0') strDescriptionKey += '_' + asset.instanceid;
      ...
      if (typeof g_bIsInMarketplace != 'undefined' && g_bIsInMarketplace) {
        if (typeof description.marketable == 'undefined' || !description.marketable) continue;
      }

      asset.description = description;
      asset.description.use_count++;

      if (asset.is_currency) this.m_rgCurrencies[asset.currencyid] = asset;
      else this.m_rgAssets[asset.assetid] = asset;
  ...
  if (this.m_bNeedsRepagination) {
    if (this.m_bActive) {
      ...
      g_ActiveInventory &&
      g_ActiveInventory.m_appid == this.m_appid &&
      g_ActiveInventory.contextid == APPWIDE_CONTEXT
    ) {
...
if (IS_ENABLED_SIH) {
  setTimeout(function () {
      ...
      if (selectmode && !isStack) {
        if (
          ...
          (this.rgItem.description.marketable ||
            ...
            (this.rgItem.blockExpiredAt && includeTradelocked))
        ) {
          ...
          if (e.ctrlKey) {
            $J('.inventory_ctn:visible .inventory_page .item').each(function (i, el) {
              if (
                (this.rgItem.description.marketable || (this.rgItem.blockExpiredAt && includeTradelocked)) &&
    ...
    $J('.item_desc_description').on('click', '.floatbutton', async function () {
      const $btn = $J(this);
      ...
      const updateFloatValue = function (data) {
        if (Array.isArray(CSGO_ORIGINS) && CSGO_ORIGINS.length && data.iteminfo) {
          ...
          const hasKeychain = data.iteminfo.keychains?.length;

          const $floatBlock = $J(`
                    <div class="float_block received">
                        <div class="itemfloat">${SIHLang.float_block.float}: <span>${data.iteminfo.floatvalue.toFixed(16)}</span></div>
                        <div class="itemseed">${SIHLang.float_block.paintseed}: <span>${data.iteminfo.paintseed}</span></div>
                        ${
                          hasKeychain
                            ? `<div class="charmseed">${SIHLang.float_block.charm_paintseed}: <span>${data.iteminfo.keychains[0].pattern}</span></div>`
                            : ''
                        }
                        <a href="https://sih.app/floats" target="_blank" class="item-position" title="${SIHLang.position_tooltip}"></a>
                        <div class="itemorigin">${SIHLang.float_block.origin}: <span>${itemOrigin.name}</span></div>
                        <div class="float-bar">
                          <div class="float-indicator" style="left: ${data.iteminfo.floatvalue * 300 - 5}px"></div>
                        </div>
                    <div>
                `);
          ...
          if (data.iteminfo.globalRatingPos || data.iteminfo.localRatingPos) {
            ...
            if (data.iteminfo.globalRatingPos) {
              const $globalPositionBlock = $J(/* HTML */ `
                <div class="item-position__global">
                  <span class="text">${SIHLang.float_block.global}: </span>
                  <span class="value value_yellow">
                    ${data.iteminfo.globalRatingPos > 0
                      ? `${data.iteminfo.globalRatingPos} ${SIHLang.float_block.low}`
                      : `${Math.abs(data.iteminfo.globalRatingPos)} ${SIHLang.float_block.high}`}
                  </span>
                </div>
              `).css('display', 'flex');

              $floatBlock.find('.item-position').append($globalPositionBlock);
            }

            if (data.iteminfo.localRatingPos) {
              const $localPositionBlock = $J(/* HTML */ `
                <div class="item-position__local">
                  <span class="text">${SIHLang.float_block.local}: </span>
                  <span class="value value_yellow">${data.iteminfo.localRatingPos}</span>
                </div>
              `).css('display', 'flex');

              $floatBlock.find('.item-position').append($localPositionBlock);
            }
          }

          $parent.html($floatBlock);
          ...
          if (g_ActiveInventory.selectedItem.description.descriptions[2].isFloat) {
            g_ActiveInventory.selectedItem.description.descriptions[2].value = $floatBlock.prop('outerHTML');
          }
        }
      };
      ...
      if (!rgItem.floatvalue) {
        ...
        if (hasKeychain && !keychains) {
          ...
          updateFloatValue({ iteminfo: { floatvalue, paintseed, origin, globalRatingPos, localRatingPos, keychains } });
        }
      }
    });
...
var getSetLink = function (d, sItem, isGenuine) {
  ...
  if (cachePrices[setLink] && cachePrices[setLink].lowestPrice) {
    ...
    if (sItem === g_ActiveInventory.selectedItem.description) reloadDes();
    return;
  } else {
...
var getLowestPriceHandler = function (gitem, callback, isSale = false) {
  ...
  var itemLink =
    ...
    sItem.appid +
    '&market_hash_name=' +
    encodeURIComponent(sItem.description.market_hash_name);
  var itemLinkW =
    window.location.protocol +
    '//steamcommunity.com/market/priceoverview/?appid=' +
    sItem.appid +
  ...
  if (+sItem.appid === 730 && !isStack && isWeapon && sItem.description.actions && sItem.description.actions.length) {
    const btnGetFloat = `
        ...
        PriceQueue.GetPrice({
          ...
          success: function (response, $this) {
            ...
            if (response.success) {
              ...
              if (sItem === g_ActiveInventory.selectedItem) reloadDes();
            }
          },
          error: function () {},
        });
  ...
  if (sItem.description.lowestPrice && diffDatePrice < +window.priceUpdateTime) {
    if (g_bViewingOwnProfile) {
      if (
        !g_ActiveInventory.selectedItem.countGroupInventory ||
        g_ActiveInventory.selectedItem.countGroupInventory === 1
      ) {
    ...
    getLowestPriceHandler(
      ...
      function (item) {
        ...
        if (item && item.description.lowestPriceW) {
          ...
          $J('.queue-total-price .with-fee').html(
            v_currencyformat(qTotalPrice, GetCurrencyCode(g_rgWalletInfo.wallet_currency))
          );
          $J('.queue-total-price .without-fee').html(
            v_currencyformat(qTotalBuyerPrice, GetCurrencyCode(g_rgWalletInfo.wallet_currency))
          );
          cb();
        } else {
          $J('#' + item.appid + '_' + item.contextid + '_' + item.assetid + '.selectedSell').removeClass(
            'selectedSell'
          );
...
const QuickSellForInventoryItem = (({
  ...
  const createSellItemButton = (item, sellingPrice, data) => {
    ...
    const btn = CreateMarketActionButton('green', 'javascript:void(0);', SIHLang.quicksell.replace('$1', sellingPrice));
    btn.id = btnId;
    $J(btn).click(() => {
      $elPriceInfoContent.html(
        '<img src="' +
          window.location.protocol +
  ...
  const calculatePrices = (item) => {
    ...
    if (price <= 0) {
      ...
      sellingPrice = CalculateAmountToSendForDesiredReceivedAmount(price, publisherFee).amount;
    }

    return {
      price,
      sellingPrice: v_currencyformat(sellingPrice, GetCurrencyCode(g_rgWalletInfo.wallet_currency)),
    };
  };
  ...
  return {
    addButton(item, container) {
      ...
      $J(container)
        .find('.item_market_action_button')[0]
        ?.after(createSellItemButton({ ...item }, sellingPrice, data));
    },
...
var ModifyDescriptionFunction = function () {
  //TODO разобраться в loadRightInventoryPanel
  //Для первой загрузки стим инвентаря
  if (g_ActiveInventory.selectedItem) {
    const hoverItemOptions = {
      item: $J('.inventory_page_right .hover_item_name'),
      marketHashName: g_ActiveInventory.selectedItem.description.market_hash_name,
    };

    SIH?.global?.ModifyHoverItemName(hoverItemOptions);
  }
  ...
  PopulateDescriptions = function (elDescriptions, rgDescriptions) {
    ...
    // if (g_ActiveInventory.selectedItem) {
    //   SIH?.itemDescription?.AddSihAppLinkToDesc(g_ActiveInventory.selectedItem);
    // }
    ...
    LoadCompleteAllInventory().then((res) => {
      const hoverItemOptions = {
        item: $J('.inventory_page_right .hover_item_name'),
        marketHashName: g_ActiveInventory.selectedItem.description.market_hash_name,
      };
    ...
    const playerItems = new Set(
      Object.keys(g_ActiveInventory.m_rgAssets).map((item) => {
    ...
    for (var i = 0; i < rgDescriptions.length; i++) {
      ...
      if (description.color) elDescription.style.color = '#' + description.color;
      ...
      if (strParsedDescription.length == 0) {
        ...
        if (description.color && description.color === '6c7075') {
          if (
            playerItems.has(description.value) ||
            [...playerItems].find((item) => item.endsWith(description.value) || description.value.endsWith(item))
          ) {
            html = '&#10003;&nbsp;' + html;
          } else {
            html = '&#10007;&nbsp;' + html;
    ...
    if (
      window.hidden_item_description &&
      g_ActiveInventory &&
      +g_ActiveInventory.appid !== 570 &&
      +g_ActiveInventory.appid !== 440
    ) {
      SIH?.itemDescription?.RenderHideDescriptionsBtn(elDescriptions, rgDescriptions);
    }
  };

  const orgPopulateTags = PopulateTags;
  PopulateTags = function (elTags, elTagsContent, rgTags) {
    ...
    if (
      ...
      $J(elTags).addClass('sih__desc-element sih__desc-element_hidden');
    }
  };
};
    ...
    floatQueue.map(({ element, link }) =>
      GetFloat(link).then((data) => {
        ...
        if (!$curItem.find('.item-info .float-value').length) {
          ...
          $curItem.find('.item-info .float-data').append(`
             <div class="item_row item_row__transparent"><span>${SIH?.phaseOfItems?.GetRatingPos(rgItem)}</span><span class="paintseed">${data.iteminfo.paintseed}</span></div>
    ...
    $J.each(ExternalPrices[appid].apis, (idx, elem) => {
      ...
      if (elem.api && elem.api.GetPrices && curSources.includes(it.id)) {
        ...
        timer = setInterval(() => {
          ...
          if (
            $itemPriceContainer ===
            (g_ActiveInventory.m_cItems || Object.keys(g_ActiveInventory.m_rgDescriptions).length)
          ) {
            clearInterval(timer);
            cnt += 1;
            elem.api.GetPrices(
...
var CancelSelectAll = function () {
  ...
  $J(`.inventory_page div.item.app${g_ActiveInventory.appid}`).each((idx, elem) => {
...
var ItemTreadLocks =
  ...
  (() => {
    ...
    const renderItem = (item, badge, locked) => {
      ...
      item.append(`
            <div class="trade-lock-badge trade-lock-badge__${locked ? 'red' : 'yellow'}">${badge}</div>
        `);
    };
    ...
    const updateInventoryItems = (items, additionalData) => {
      ...
      items.each(function (index, el) {
        ...
        const { cache_expiration, tradable, marketable } = additionalData[`${classid}_${instanceid}`] || {};
    ...
    return {
      add: async () => {
        try {
          ...
          updateInventoryItems($J(`div.item.app${appid}`), data.rgDescriptions);
        } catch (error) {
