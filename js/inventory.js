...
if (IS_ENABLED_SIH) {
  ...
  var PopulateMarketActions = (elActions, item) => {
    ...
    if (!item.description.marketable && item.blockExpiredAt) {
  ...
  Ajax.Request = function (url, data) {
    ...
    if (url.indexOf('/market/priceoverview') > -1) {
      ...
      $J(g_elActions).find('.sih-instant-sell').remove();
  ...
  if (g_inventoryData[g_ActiveInventory.appid] && !g_inventoryData[g_ActiveInventory.appid].isLoadFirst) {
    ...
    LoadCompleteAllInventory().then(() => {
      ...
      if (g_steamID === g_ActiveInventory.m_owner.strSteamId) {
        ...
        loadProfileInventory(g_steamID).done(() => {
          ItemTreadLocks.add();
...
function loadProfileInventory(steamId) {
  ...
  return $J.get(url, params).done(function (data) {
...
function loadInstantSellForInventoryItem(rgItem) {
  if (g_ActiveInventory.selectedItem && !$J(g_ActiveInventory.selectedItem.element).data('selling')) {
    const bIsTrading = typeof g_bIsTrading != 'undefined' && g_bIsTrading;
    if (!selectmode && !bIsTrading) {
      SIH?.itemDescription?.instantSellBtn?.RenderInstantSellButton(rgItem, g_elActions);
    }
  }
}
  ...
  if (isEnabledGemApi && +g_ActiveInventory.appid === 753) {
    setTimeout(async () => {
      ...
      for (const elem of $J('.selectedSell').toArray()) {
        ...
        if (rgItem.goo_value === undefined && rgItem.description.marketable) {
...
function ajaxDataLoadingProcess(isLoad, marketHashName = '') {
  ...
  if ($elem.length === 0) {
    const cnt = $J(
      '<div id="_priceQueueContV2" class="pq-container"><div class="pq-timer"><div class="pq-progress">&nbsp;</div></div><div class="pq-info">&nbsp;</div></div>'
    );
    $J('body').append(cnt);
  }
  ...
  if (isLoad) {
    ...
    $elem.find('.pq-info').html(`${DOMPurify.sanitize(marketHashName)} <br /> 1 items remain - 0 errors`);
    $elem.show();
  } else {
    $elem.hide();
  }
...
function copyDuplicateInventory(rgItem) {
  ...
  const item = rgAssets.find(
    (item) => item.description.market_hash_name === rgItem.description.market_hash_name && item.description.lowestPrice
  );
